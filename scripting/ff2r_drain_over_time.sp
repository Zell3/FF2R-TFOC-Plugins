...
float DOT_NextTick;
bool  DOT_CanUse[MAXPLAYERS + 1];
float DOT_TimeOfLastSound[MAXPLAYERS + 1];
bool  DOT_ReloadDown[MAXPLAYERS + 1];
bool  DOT_RageActive[MAXPLAYERS + 1];
int   DOT_ActiveTickCount[MAXPLAYERS + 1];
bool  DOT_OverlayVisible[MAXPLAYERS + 1];
bool  DOT_ActivationCancel[MAXPLAYERS + 1];
bool  DOT_ForceDeactivation[MAXPLAYERS + 1];
bool  DOT_Usable[MAXPLAYERS + 1];
bool  DOT_IsOnCooldown[MAXPLAYERS + 1];
int   DOT_CooldownTicksRemaining[MAXPLAYERS + 1];
bool  DOT_ReloadPressPending[MAXPLAYERS + 1];
float DOT_MinRage[MAXPLAYERS + 1];                              // arg1
float DOT_RageDrain[MAXPLAYERS + 1];                            // arg2
float DOT_EnterPenalty[MAXPLAYERS + 1];                         // arg3
float DOT_ExitPenalty[MAXPLAYERS + 1];                          // arg4
char  DOT_EntrySound[MAXPLAYERS + 1][MAX_SOUND_FILE_LENGTH];    // arg5
char  DOT_ExitSound[MAXPLAYERS + 1][MAX_SOUND_FILE_LENGTH];     // arg6
char  DOT_EntryEffect[MAXPLAYERS + 1][MAX_EFFECT_NAME_LENGTH];  // arg7
float DOT_EntryEffectDuration[MAXPLAYERS + 1];                  // arg8
char  DOT_ExitEffect[MAXPLAYERS + 1][MAX_EFFECT_NAME_LENGTH];   // arg9: Rage exit particle effect
float DOT_ExitEffectDuration[MAXPLAYERS + 1];                   // arg10: Duration of said particle effect
int   DOT_ConditionChanges[MAXPLAYERS + 1][MAX_CONDITIONS];     // arg11: Conditions to add (and then subsequently remove) during the reload-activated rage.
bool  DOT_NoOverlay[MAXPLAYERS + 1];                            // arg12: Don't use overlay
int   DOT_CooldownDurationTicks[MAXPLAYERS + 1];                // arg13: Tick count for cooldown
int   DOT_ActivationKey[MAXPLAYERS + 1];                        // arg14: Activation key (IN_RELOAD or IN_ATTACK3)
bool  DOT_AllowWhileStunned[MAXPLAYERS + 1];                    // arg15
...
public Action Timer_PostRoundStartInits(Handle timer)
{
  ...
  for (int clientIdx = 1; clientIdx < MaxClients; clientIdx++)  // make no boss count assumptions, though anything above 3 is very weird
  {
    ...
    if (conditionStr[0] != '\0')
    {
      ...
      if (StrContains(conditionStr, " ; ") > 0)
        conditionCount = ExplodeString(conditionStr, " ; ", conditions, MAX_CONDITIONS, 4);
