...
public void OnRoundStart(Event event, const char[] name, bool dontBroadcast)
{
  ...
  for (int boss; boss <= MaxClients; boss++)
  {
    ...
    if (FF2_HasAbility(boss, this_plugin_name, BLITZTIMER))
    {
      PrintToServer("Blitz Timer");
...
public bool LoadPhrases(int boss)
{
  ...
  Enabled = true;
	BossData cfg = FF2R_GetBossData(boss);	// Get boss config (known as boss index) from player
  if (!cfg)
    return false;
  
  AbilityData ability = cfg.GetAbility("special_phrases");
  if (ability.IsMyPlugin())
  {
    char phrase[MAX_TRANS_LENGTH];
    ability.GetString("file", phrase, sizeof(phrase));
    if (phrase[0] != '\0')
    {
