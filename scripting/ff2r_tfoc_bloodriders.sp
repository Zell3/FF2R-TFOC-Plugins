...
char   hookbossargs[MAX_WEAPON_ARG_LENGTH];
char   hookplayersargs[MAX_WEAPON_ARG_LENGTH];
...
int    clientRevives[MAXPLAYERS + 1];
...
char   ScoutSpeed[MAXPLAYERS + 1];
char   SoldierSpeed[MAXPLAYERS + 1];
char   PyroSpeed[MAXPLAYERS + 1];
char   DemoSpeed[MAXPLAYERS + 1];
char   HeavySpeed[MAXPLAYERS + 1];
char   EngineerSpeed[MAXPLAYERS + 1];
char   MedicSpeed[MAXPLAYERS + 1];
char   SniperSpeed[MAXPLAYERS + 1];
char   SpySpeed[MAXPLAYERS + 1];
...
// Internal stuff
bool   Raging[MAXPLAYERS + 1];
bool   Nextlive[MAXPLAYERS + 1];
...
public Action OnAnnounce(Handle event, const char[] name, bool dontBroadcast)
{
  if (bloodisboss)
  {
    char strAudio[40];
  ...
  for (int clientIdx = 1; clientIdx <= MaxClients; clientIdx++)
  {
    ...
    if (bossIdx >= 0)
    {
      if (FF2_HasAbility(bossIdx, this_plugin_name, CONFIG_BLOOD))
      {
        ...
        grapplinkhookboss       = view_as<bool>(FF2_GetAbilityArgument(bossIdx, this_plugin_name, CONFIG_BLOOD, 2));
        FF2_GetAbilityArgumentString(bossIdx, this_plugin_name, CONFIG_BLOOD, 3, hookbossargs, MAX_WEAPON_ARG_LENGTH);

        grapplinkhookplayers = view_as<bool>(FF2_GetAbilityArgument(bossIdx, this_plugin_name, CONFIG_BLOOD, 4));
        FF2_GetAbilityArgumentString(bossIdx, this_plugin_name, CONFIG_BLOOD, 5, hookplayersargs, MAX_WEAPON_ARG_LENGTH);
        givehookback = view_as<bool>(FF2_GetAbilityArgument(bossIdx, this_plugin_name, CONFIG_BLOOD, 6));
        ...
        Waveenabled           = view_as<bool>(FF2_GetAbilityArgument(bossIdx, this_plugin_name, CONFIG_BLOOD, 13));
...
public void FF2R_OnBossCreated(int client, BossData cfg, bool setup)
{
  ...
  {
    AbilityData ability = cfg.GetAbility(CONFIG_BLOOD);
...
public void OnPlayerDeath(Event event, const char[] name, bool dontBroadcast)
{
  ...
  {
    bool  AllowgainingRage = view_as<bool>(FF2_GetAbilityArgument(BloodriderBossIdx, this_plugin_name, LIFELOSE_BLOOD, 7));
    ...
    bool  AllowgainingHealth = view_as<bool>(FF2_GetAbilityArgument(BloodriderBossIdx, this_plugin_name, RAGE_BLOOD, 6));
  ...
  if (bloodisboss)
  {
    ...
    if (AdditionalVoiceOvers == 1 || AdditionalVoiceOvers == 3)
    {
      if (GetEventInt(event, "winning_team") == FF2_GetBossTeam())
        BossIsWinner = true;
      else if (GetEventInt(event, "winning_team") == ((FF2_GetBossTeam() == view_as<int>(TFTeam_Blue)) ? (view_as<int>(TFTeam_Red)) : (view_as<int>(TFTeam_Blue))))
        BossIsWinner = false;
    }
...
public void FF2R_OnAbility(int client, const char[] ability_name, AbilityData cfg)
{
  ...
  {
    TF2_AddCondition(client, TFCond_Buffed, GetTimerDuration(client, ability_name, 1, false));                  // Minicrits
    TF2_AddCondition(client, TFCond_DefenseBuffNoCritBlock, GetTimerDuration(client, ability_name, 1, false));  // Defensive Buff
    TF2_AddCondition(client, TFCond_RegenBuffed, GetTimerDuration(client, ability_name, 1, false));             // Regen Buff

    int grenade = FF2_GetAbilityArgument(client, this_plugin_name, ability_name, 2);  // Ammo

    SwitchSouls(client, grapplinkhookboss, Waveenabled, true);
    SetAmmo(client, TFWeaponSlot_Primary, grenade);

    NewGrenadeTimer        = GetTimerDuration(client, ability_name, 3, true);
    Raging[client]         = true;

    float reflectionchance = FF2_GetAbilityArgumentFloat(client, this_plugin_name, ability_name, 4);
    if (reflectionchance > 0.0)
    {
      if (GetRandomFloat(0.0, 1.0) <= reflectionchance)
      {
        TF2_AddCondition(client, TFCond_RuneWarlock, GetTimerDuration(client, ability_name, 5, false));  // Warlock
      }
    }
  ...
  {
    TF2_AddCondition(client, TFCond_HalloweenCritCandy, GetTimerDuration(client, ability_name, 1, false));  // Crits
    TF2_AddCondition(client, TFCond_Ubercharged, GetTimerDuration(client, ability_name, 1, false));         // Ubercharge
    Blood_RemoveUberAt = GetTimerDuration(client, ability_name, 1);
    SetEntProp(client, Prop_Data, "m_takedamage", 0);

    int lifelosegrenade = FF2_GetAbilityArgument(client, this_plugin_name, ability_name, 2);  // Ammo
    ...
    switch (GetRandomInt(0, 3))
    {
      case 0:
      {
        TF2_AddCondition(client, TFCond_RuneStrength, GetTimerDuration(client, ability_name, 4, false));  // Strength
      }
      case 1:
      {
        TF2_AddCondition(client, TFCond_RuneHaste, GetTimerDuration(client, ability_name, 4, false));  // Haste
      }
      case 2:
      {
        TF2_AddCondition(client, TFCond_RunePrecision, GetTimerDuration(client, ability_name, 4, false));  // Precision
      }
      case 3:
      {
        TF2_AddCondition(client, TFCond_RuneAgility, GetTimerDuration(client, ability_name, 4, false));  // Agility
      }
    }
        ...
        switch (weapondifficulty)
        {
          case 11:
          {
            if (IsRandomDifficultyMode)
              ...
              weapondifficulty = FF2_GetAbilityArgument(client, this_plugin_name, CONFIG_BLOOD, 1);
          }
...
public void Blood_HUDSyncTick(float curTime)
{
  if (curTime >= Bloodrider_FindBloodriderAt)
  {
    ...
    for (int clientIdx = 1; clientIdx <= MaxClients; clientIdx++)
    {
      ...
      {
        RegenerateLivesOn    = view_as<bool>(FF2_GetAbilityArgument(bossIdx, this_plugin_name, CONFIG_BLOOD, 7));
        IntroOutroOn         = view_as<bool>(FF2_GetAbilityArgument(bossIdx, this_plugin_name, CONFIG_BLOOD, 8));
    ...
    for (int clientIdx = 1; clientIdx <= MaxClients; clientIdx++)
    {
      ...
      if (!BloodCount)
      {
        ...
        if (IsPlayerAlive(clientIdx) && !IsBoss(clientIdx))
        {
          ...
          CPrintToChat(clientIdx, "{orange}[TFOC]{default} You have earned %i queue points for surviving a wave of %i gernades for %i seconds", qPoints, grenadeCount, BloodTimePassed);
    ...
    if (BloodCount <= 10)
    {
      ...
      switch (BloodCount)
      {
        case 0:  // Give ammo & reset timer
        {
          ...
          BloodTimePassed = 0;
          Blood_WaveTick += 1.0;
          PrintToChatAll("\x04[The Bloodrider] \x01Waves %i / %i : Next wave in %i seconds", wavesDone, BloodMaxWaves, BloodCount);
        ...
        {
          if (BloodCount == 10)
            PrintToChatAll("\x04[The Bloodrider] \x01Next wave in %i seconds", BloodCount);
          if (BloodCount == 5)
            PrintToChatAll("\x04[The Bloodrider] \x01Next wave in %i", BloodCount);
...
public Action RoundResultSound(Handle hTimer)
{
...
public void OnDeflectObject(Event event, const char[] name, bool dontBroadcast)
{
  int client = GetClientOfUserId(GetEventInt(event, "userid"));
  ...
  if (Waveenabled)
  {
    ...
    {
      static int deflected[MAXPLAYERS + 1];
      static int deflect;
      ...
      if (deflect >= 5)
      {
        ...
        CPrintToChat(client, "{orange}[TFOC]{default} You have earned %i queue points for deflecting %i times", pts, deflected[client]);
        deflect -= deflected[client];
      }
      deflected[client]++;
    }
...
public void SwitchSouls(int client, bool grapplinkhook, bool waveon, bool rageisactive)
{
    ...
    case TFClass_Soldier:
    {
      if (!waveon)
      {
        switch (weapondifficulty)
        {
          ...
          case 9, 10, 11: SpawnWeapon(client, "tf_weapon_shovel", 196, 100, 5, "2025 ; 3 ; 2013 ; 2007 ; 2014 ; 4 ; 68 ; 10 ; 2 ; 10 ; 6 ; 0.5 ; 214 ; %d", Blood_meleeKills[client]);
        }
      }
    ...
    case TFClass_Pyro:
    {
      if (!waveon)
      {
        switch (weapondifficulty)
        {
          ...
          case 9, 10, 11: SpawnWeapon(client, "tf_weapon_fireaxe", 192, 100, 5, "2025 ; 3 ; 2013 ; 2007 ; 2014 ; 4 ; 68 ; 10 ; 2 ; 10 ; 6 ; 0.5 ; 214 ; %d", Blood_meleeKills[client]);
        }
      }
    ...
    case TFClass_DemoMan:
    {
      if (!waveon)
      {
        switch (weapondifficulty)
        {
          ...
          case 9, 10, 11: SpawnWeapon(client, "tf_weapon_bottle", 191, 100, 5, "2025 ; 3 ; 2013 ; 2007 ; 2014 ; 4 ; 68 ; 10 ; 2 ; 10 ; 6 ; 0.5 ; 214 ; %d", Blood_meleeKills[client]);
        }
      }
    ...
    case TFClass_Heavy:
    {
      if (!waveon)
      {
        switch (weapondifficulty)
        {
          ...
          case 9, 10, 11: SpawnWeapon(client, "tf_weapon_fists", 195, 100, 5, "2025 ; 3 ; 2013 ; 2007 ; 2014 ; 4 ; 68 ; 10 ; 2 ; 10 ; 6 ; 0.5 ; 214 ; %d", Blood_meleeKills[client]);
        }
      }
    ...
    case TFClass_Engineer:
    {
      if (!waveon)
      {
        switch (weapondifficulty)
        {
          ...
          case 9, 10, 11: SpawnWeapon(client, "tf_weapon_wrench", 197, 100, 5, "2025 ; 3 ; 2013 ; 2007 ; 2014 ; 4 ; 68 ; 10 ; 2 ; 10 ; 6 ; 0.5 ; 214 ; %d", Blood_meleeKills[client]);
        }
      }
    ...
    case TFClass_Medic:
    {
      if (!waveon)
      {
        switch (weapondifficulty)
        {
          ...
          case 9, 10, 11: SpawnWeapon(client, "tf_weapon_bonesaw", 198, 100, 5, "2025 ; 3 ; 2013 ; 2007 ; 2014 ; 4 ; 68 ; 10 ; 2 ; 10 ; 6 ; 0.5 ; 214 ; %d", Blood_meleeKills[client]);
        }
      }
    ...
    case TFClass_Sniper:
    {
      if (!waveon)
      {
        switch (weapondifficulty)
        {
          ...
          case 9, 10, 11: SpawnWeapon(client, "tf_weapon_club", 193, 100, 5, "2025 ; 3 ; 2013 ; 2007 ; 2014 ; 4 ; 68 ; 10 ; 2 ; 10 ; 6 ; 0.5 ; 214 ; %d", Blood_meleeKills[client]);
        }
      }
    ...
    case TFClass_Spy:
    {
      if (!waveon)
      {
        switch (weapondifficulty)
        {
          ...
          case 9, 10, 11: SpawnWeapon(client, "tf_weapon_knife", 194, 100, 5, "2025 ; 3 ; 2013 ; 2007 ; 2014 ; 4 ; 68 ; 10 ; 2 ; 10 ; 6 ; 0.5 ; 214 ; %d", Blood_meleeKills[client]);
        }
      }
...
public void Grenadelauncher(int client)
{
  int index;
...
public void Grenadelauncherrage(int client)
{
  int  index;
...
public void GrapplinkhookForPlayers()
{
  for (int i = 1; i <= MaxClients; i++)
  {
...
stock void RefreshDifficulty(int level)
{
  ...
  for (int client = 1; client <= MaxClients; client++)
  {
    ...
    if (level == 1)
    {
      Bloodrider_DifficultyLevelString[client] = BLOOD_Easy[client];
      CPrintToChat(client, "{lime}[The Bloodrider]{default} Difficulty - %s", BLOOD_Easy[BossIdx]);
    }
    else if (level == 2) {
      Bloodrider_DifficultyLevelString[client] = BLOOD_Normal[client];
      CPrintToChat(client, "{lime}[The Bloodrider]{default} Difficulty - %s", BLOOD_Normal[BossIdx]);
    }
    else if (level == 3) {
      Bloodrider_DifficultyLevelString[client] = BLOOD_Intermediate[client];
      CPrintToChat(client, "{lime}[The Bloodrider]{default} Difficulty - {yellow}%s{default}", BLOOD_Intermediate[BossIdx]);
    }
    else if (level == 4) {
      Bloodrider_DifficultyLevelString[client] = BLOOD_Difficult[client];
      CPrintToChat(client, "{lime}[The Bloodrider]{default} Difficulty - {red}%s{default}", BLOOD_Difficult[BossIdx]);
    }
    else if (level == 5) {
      Bloodrider_DifficultyLevelString[client] = BLOOD_Lunatic[client];
      CPrintToChat(client, "{black}[ブラッドライダー]{unusual} Difficulty - %s{default}", BLOOD_Lunatic[BossIdx]);
    }
    else if (level == 6) {
      Bloodrider_DifficultyLevelString[client] = BLOOD_Insane[client];
      CPrintToChat(client, "{lime}[The Bloodrider]{default} Difficulty - {valve}%s{default}", BLOOD_Insane[BossIdx]);
    }
    else if (level == 7) {
      Bloodrider_DifficultyLevelString[client] = BLOOD_Godlike[client];
      CPrintToChat(client, "{lime}[The Bloodrider]{default} Difficulty - {gold}%s{default}", BLOOD_Godlike[BossIdx]);
    }
    else if (level == 8) {
      Bloodrider_DifficultyLevelString[client] = BLOOD_GrenadeHell[client];
      CPrintToChat(client, "{lime}[The Bloodrider]{default} Difficulty - {darkgreen}%s{default}", BLOOD_GrenadeHell[BossIdx]);
    }
    else if (level == 9) {
      Bloodrider_DifficultyLevelString[client] = BLOOD_TrueBloodrider[client];
      CPrintToChat(client, "{lime}[The Bloodrider]{default} Difficulty - {lime}%s{default}", BLOOD_TrueBloodrider[BossIdx]);
    }
    else if (level == 10) {
      Bloodrider_DifficultyLevelString[client] = BLOOD_TrueBloodrider[client];
      CPrintToChat(client, "{purple}[ブラッドライダー]{default} Kakera no Majo - {black}ベルンカステル{default}");
    }
    else if (level == 11) {
      Bloodrider_DifficultyLevelString[client] = BLOOD_TrueBloodrider[client];
      CPrintToChat(client, "{purple}[ブラッドライダー ~ The Bloodrider]{gold} TFOC Salute You!{default}");
    }
    else {
      Format(Bloodrider_DifficultyLevelString[client], sizeof(Bloodrider_DifficultyLevelString[]), BLOOD_RNGDisplay[client], level);
    }
  }
}

stock void ReadCenterText(int bossIdx, const char[] ability_name, int argInt, char centerText[PLATFORM_MAX_PATH])
{
...
stock void DropReviveMarker(int client)
{
  switch (MaxClientRevives)
  {
    ...
    {
      static int revivecount[MAXPLAYERS + 1];
