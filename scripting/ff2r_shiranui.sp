/*
  "shiranui"
  {
    ...
    "hud_norage"        "Not enough rage to dominate! (%.0f%% needed)"    // Text when not enough rage
    "hud_norage_color"  "255 ; 255 ; 255 ; 255"   // Text color when not enough rage (rgba : 0-255)
    "hud_ready"         "Dominate is Ready! (Reload to use)"    // Text when ability is ready
...
public void FF2R_OnBossCreated(int client, BossData cfg, bool setup)
{
  ...
  {
    AbilityData ability = cfg.GetAbility("shiranui");
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon, int &subtype, int &cmdnum, int &tickcount, int &seed, int mouse[2])
{
  ...
  AbilityData ability = boss.GetAbility("shiranui");
  ...
  if (g_hHudSync != null && g_bHudEnabled[client])
  {
    ...
    if (lastman && GetTeamClientAliveCount(enemyTeam) == 1)
    {
      ...
      ShowSyncHudText(client, g_hHudSync, "You are not allowed to dominate the last player!");
    }
    else if (charge >= cost)
    {
  ...
  if (buttons & IN_RELOAD)
  {
    ...
    if (charge >= cost)
    {
      ...
      {
        PrintHintText(client, "You dominated %N!", target);
        PrintCenterText(target, "You are dominated!");
      ...
      else if (isBuilding && buildingHack)
      {
        ...
        if (IsValidClient(builder) && GetClientTeam(builder) == view_as<int>(enemyTeam))
        {
          ...
          PrintHintText(client, "You dominated %N's building!", builder);
          PrintCenterText(builder, "Your building is dominated!");
    ...
    {
      PrintHintText(client, "Not enough rage to dominate! (%.0f%% needed)", cost, charge);
    }
  }
  return Plugin_Continue;
}
