...
bool  DT_IsCanTeleport[MAXTF2PLAYERS];
...
public void
  FF2R_OnBossCreated(int client, BossData cfg, bool setup)
{
...
public void OnDOTAbilityTick(int clientIdx, int tickCount)
{
  ...
  // Teleport
  if (DT_CanUse[clientIdx] && DT_IsCanTeleport[clientIdx])
  {
		DT_IsCanTeleport[clientIdx] = false;  // reset the teleport flag so it can be used again
...
public bool Teleport(int clientIdx)
{
  ...
  TR_TraceRayFilter(startPos, eyeAngles, MASK_PLAYERSOLID, RayType_Infinite, TracePlayersAndBuildings);
  TR_GetEndPosition(endPos);

  float distance = GetVectorDistance(startPos, endPos);

  if (distance > DT_MaxDistance[clientIdx])
    constrainDistance(startPos, endPos, distance, DT_MaxDistance[clientIdx]);
  ...
  for (int x = 0; x < 3; x++)
  {
    ...
    for (int y = 0; y < 3; y++)
    {
      ...
      for (int z = 0; z < 3; z++)
      {
        ...
        TR_TraceRayFilter(endPos, testPos, MASK_PLAYERSOLID, RayType_EndPoint, TraceWallsOnly);
  ...
  if (!found)
  {
	...
	DT_IsCanTeleport[clientIdx] = true;  // allow the teleport to be used
  ...
  ParticleEffectAt(testPos, DT_NewLocationParticleEffect[clientIdx]);
