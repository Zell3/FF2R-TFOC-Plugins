...
public SO_PreThink(clientIdx)
{
	if (IsLivingPlayer(clientIdx) && SO_CanUse[clientIdx] && SO_ActiveThisRound)
	{
		...
		if (SO_MaxHP[clientIdx] > 0)
		{
			...
			#endif
			new Float:baseSpeed = SO_LowSpeed[clientIdx] + ((1.0 - (float(hp) / float(SO_MaxHP[clientIdx]))) * (SO_HighSpeed[clientIdx] - SO_LowSpeed[clientIdx]));
...
public LC_RemoveCurse(victim)
{
	...
	if (LC_ParticleEntRef[victim] != INVALID_ENTREF)
		SarysaRemoveEntity(INVALID_HANDLE, LC_ParticleEntRef[victim]);
...
public DD_Cleanup()
{
	...
	for (new gemIdx = 0; gemIdx < MAX_GEMS; gemIdx++)
	{
		if (DDG_EntRef[gemIdx] != INVALID_ENTREF)
			SarysaRemoveEntity(INVALID_HANDLE, DDG_EntRef[gemIdx]);
		DDG_EntRef[gemIdx] = INVALID_ENTREF;
	}
}
...
public DD_OnGemCollide(gemIdx, clientIdx, victim)
{
	if (DD_Flags[clientIdx] & DD_FLAG_HOOKED_DAMAGE)
		FullyHookedDamage(victim, clientIdx, fixDamageForFF2(DD_GemDamage[clientIdx] / 3), DMG_CRIT);
	else
...
stock BH_Blink(clientIdx, bool:calledByDOT = false, bool:force = false)
{
	if (BH_BlinkReady[clientIdx] && (force || GetEngineTime() >= BH_BlinkAllowedAt[clientIdx]))
	{
		...
		{
			SarysaRemoveEntity(INVALID_HANDLE, BH_ProjectileEntRef[clientIdx]);
			BH_ProjectileEntRef[clientIdx] = INVALID_ENTREF;
		}
...
public RE_DisplayCastingParticle(clientIdx)
{
	...
	if (!IsEmptyString(RE_CastingParticle))
	{
		...
		if (IsValidEntity(particle))
			CreateTimer(1.0, SarysaRemoveEntity, EntIndexToEntRef(particle), TIMER_FLAG_NO_MAPCHANGE);
	}
}
...
public MSB_Tick(clientIdx, buttons, Float:curTime)
{
	...
	if (curTime >= MSB_UpdateHUDAt[clientIdx] && (buttons & IN_SCORE) == 0)
	{
		...
		SetHudTextParams(-1.0, MSB_HUDReplacementY[clientIdx], MSB_HUD_INTERVAL + 0.05, 255, 255, 255, 255);
				...
				new hp = GetEntProp(clientIdx, Prop_Send, "m_iHealth"); // see my rant about this at the bottom of this file.
		#endif
		ShowSyncHudText(clientIdx, MSB_HUDReplaceHandle, MSB_HUDReplacementFormat, FF2_GetBossCharge(bossIdx, 0), hp, FF2_GetBossMaxHealth(bossIdx));
	}
}
...
stock ParticleEffectAt(Float:position[3], String:effectName[], Float:duration = 0.1)
{
	...
	if (particle != -1)
	{
		...
		if (duration > 0.0)
			CreateTimer(duration, SarysaRemoveEntity, EntIndexToEntRef(particle), TIMER_FLAG_NO_MAPCHANGE);
	}
	return particle;
}
...
public Action:SarysaRemoveEntity(Handle:timer, any:entid)
{
...
stock SemiHookedDamage(victim, inflictor, attacker, Float:damage, damageType=DMG_GENERIC, weapon=-1)
{
	if (GetClientTeam(victim) != BossTeam)
		...
		FullyHookedDamage(victim, attacker, damage, damageType);
}
...
stock FullyHookedDamage(victim, attacker, Float:damage, damageType=DMG_GENERIC, Float:attackPos[3] = NULL_VECTOR)
{
	...
	if (IsValidEntity(pointHurt))
	{
		...
		SarysaRemoveEntity(INVALID_HANDLE, EntIndexToEntRef(pointHurt));
	}
}
...
public Float:GetWeaponAttribute(weapon, attribute, Float:defaultValue)
{
	...
	if (addr < view_as<Address>(Address_MinimumValid))
		return defaultValue;
...
CreateRagdoll(client, Float:flSelfDestruct=0.0)
{
	...
	if (iRag > MaxClients && IsValidEntity(iRag))
	{
		...
		if (flSelfDestruct > 0.0)
			CreateTimer(flSelfDestruct, SarysaRemoveEntity, EntIndexToEntRef(iRag), TIMER_FLAG_NO_MAPCHANGE);
...
public ForceUserToTaunt(clientIdx, itemdef)
{
	...
	if (pEconItemView <= view_as<Address>(Address_MinimumValid))
	{
		if (!congaFailurePrintout)
		{
