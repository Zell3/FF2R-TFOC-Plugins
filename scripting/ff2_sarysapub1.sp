...
public MonitorRocket(clientIdx, rocket, Float:velocity)
{
	...
	if (!IsEmptyString(RWI_ParticleOverride[spec]))
	{
		...
		if (IsValidEntity(particle))
			CreateTimer(10.0, RemoveEntitySarysa, EntIndexToEntRef(particle), TIMER_FLAG_NO_MAPCHANGE); // sanity timer
	}
...
public DestroyProp(propIdx, bool:reorder)
{
	...
	if (IsValidEntity(prop))
		RemoveEntitySarysa(INVALID_HANDLE, prop);
...
stock ParticleEffectAt(Float:position[3], String:effectName[], Float:duration = 0.1)
{
	...
	if (particle != -1)
	{
		...
		if (duration > 0.0)
			CreateTimer(duration, RemoveEntitySarysa, EntIndexToEntRef(particle), TIMER_FLAG_NO_MAPCHANGE);
	}
	return particle;
}
...
public Action:RemoveEntitySarysa(Handle:timer, any:entid)
{
...
stock Float:RandomNegative(Float:number)
{
	return number * (GetRandomInt(0, 1) == 1 ? 1.0 : -1.0);
}
