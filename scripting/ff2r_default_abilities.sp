...
public void OnPluginStart()
{
  ...
  PrepCall_AddParameter(Type_CBaseEntity, SDKPass_Pointer);
...
float GetPlayerStunMulti(int client)
{
  ...
  multi *= Attributes_FindOnPlayer(client, 60, true, 1.0) * Attributes_FindOnPlayer(client, 61, true, 1.0) * Attributes_FindOnPlayer(client, 64, true, 1.0) * Attributes_FindOnPlayer(client, 65, true, 1.0) * Attributes_FindOnPlayer(client, 66, true, 1.0) * Attributes_FindOnPlayer(client, 67, true, 1.0) * Attributes_FindOnPlayer(client, 412, true, 1.0) * Attributes_FindOnPlayer(client, 492, true, 1.0) * Attributes_FindOnPlayer(client, 516, true, 1.0) * Attributes_FindOnPlayer(client, 1001, true, 1.0);

  // Mark-for-Death = x1.35
  if (TF2_IsPlayerInCondition(client, TFCond_MarkedForDeath) || TF2_IsPlayerInCondition(client, TFCond_MarkedForDeathSilent) || TF2_IsPlayerInCondition(client, TFCond_PasstimePenaltyDebuff))
    multi *= 1.35;

  // Ranged damage attributes
  int active = GetEntPropEnt(client, Prop_Send, "m_hActiveWeapon");
  multi *= Attributes_FindOnWeapon(client, active, 205, true, 1.0) * Attributes_FindOnWeapon(client, active, 852, true, 1.0);

  if (TF2_IsPlayerInCondition(client, TFCond_Slowed) && health < GetClientMaxHealth(client) / 2)
...
{
  return (TF2_IsPlayerInCondition(client, TFCond_Ubercharged) || TF2_IsPlayerInCondition(client, TFCond_UberchargedCanteen) || TF2_IsPlayerInCondition(client, TFCond_UberchargedHidden) || TF2_IsPlayerInCondition(client, TFCond_UberchargedOnTakeDamage) || TF2_IsPlayerInCondition(client, TFCond_Bonked) || TF2_IsPlayerInCondition(client, TFCond_HalloweenGhostMode) || !GetEntProp(client, Prop_Data, "m_takedamage"));
}
